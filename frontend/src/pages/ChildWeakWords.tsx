...
export default function ChildWeakWords() {
  ...
  useEffect(() => {
    api.get(`/api/parent/children/${childId}/weak-words?sort_by=${sort.by}&order=${sort.order}`).then((res) => {
      setWords(res.data);
      setLoading(false);
    });
  }, [childId, sort]);
  ...
  return (
    <div className="weak-words-page">
      <div className="header">
        <h1>苦手単語一覧</h1>
        <button className="btn-secondary" onClick={() => navigate('/parent')}>戻る</button>
      </div>
      {words.length === 0 ? (
        <p>苦手単語はありません</p>
      ) : (
